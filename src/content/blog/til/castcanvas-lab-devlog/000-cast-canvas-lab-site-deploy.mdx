...
- 원인: www 리다이렉트용 CloudFront origin을 S3 website endpoint로 연결했는데, origin protocol policy가 HTTPS only로 설정되어 있었다.
S3 website endpoint는 HTTPS origin으로 붙는 구조가 아니라 HTTP origin으로 접근해야 한다.
